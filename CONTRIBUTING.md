...
### Link a GitHub Issue (Required)

Every PR **must** link a GitHub issue in the body. Use the standard `Fixes #123` / `Closes #123` / `Refs #123` syntax so GitHub auto-links it.

- If there are **duplicate or closely related issues**, link all of them — not just the one you picked. Reviewers should be able to see the full picture.
- If there are **related PRs** (prior attempts, dependent work, follow-ups, abandoned predecessors), link those too.

If no issue exists yet for what you're doing, open one first and link it.
